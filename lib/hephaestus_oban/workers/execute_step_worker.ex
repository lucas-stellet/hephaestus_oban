defmodule HephaestusOban.ExecuteStepWorker do
  ...
  @impl Oban.Worker
  def perform(%Oban.Job{args: %{"instance_id" => instance_id, "step_ref" => step_ref}} = job) do
    ...
    if StepResults.exists?(config.repo, instance_id, step_ref) do
      ...
      case Engine.execute_step(instance, step_module) do
        {:ok, event} ->
          insert_result_and_advance(config, instance_id, step_ref, to_string(event), %{})
        ...
        {:async} ->
          insert_result_and_advance(config, instance_id, step_ref, "__async__", %{})
  ...
  defp insert_result_and_advance(config, instance_id, step_ref, event, context_updates) do
